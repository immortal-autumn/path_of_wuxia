...
| Safe map schema upgrades | SQLite startup migration | Upgrades populated legacy databases despite SQLite `ALTER TABLE` foreign-key limits, rebuilds schema v5 without the former global location-name uniqueness constraint, and preserves IDs, foreign-key references and user-created records. | Unit populated-v1/v3 metadata upgrade, duplicate-name and foreign-key regression tests. |
