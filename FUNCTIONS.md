...
| Three-step neighborhood | Game map | Queries only graph depth ≤3, including portals as one step, and returns relevant regions/routes/actions. | Unit seed/neighborhood and Playwright map tests. |
...
| Layer transitions and portals | Action panel / `move` | Doors, stairs, gates and portals connect map layers without consuming direction slots; only normal routes appear in the local SVG. | Unit cross-layer transition and ordinary-route tests. |
| Six-attribute progression | Character tabs / `attributes.allocate` | Stores strength, agility, constitution, root, comprehension and spirit; atomically spends permanent points and recalculates transparent combat values. | Unit formulas, scaling rewards and atomic-allocation tests. |
| Cultivation and breakthroughs | Training locations / `cultivation.breakthrough` | Settles unlimited online/offline cultivation from server time, auto-levels within a realm, and applies 45/60/75/100% breakthroughs with failure cost and 30% carry. | Unit clock, idempotency, failure/success and reward-scaling tests. |
| Derived combat panel | Character “战斗属性” tab | Computes HP, endurance, min/max attack, defense, speed, hit, dodge, critical rate/damage and cultivation speed from attributes and realm. | Unit deterministic-formula tests. |
