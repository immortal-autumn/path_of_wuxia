...
| Continuous public overworld | `world-root` ordinary-route graph | Places the house exterior, three same-named 楼门路 street segments, all 618 sourced Northern Song nodes and all 279 sourced Palos nodes on one large chunked map. Repeated circuit/category road names represent physical street segments while stable IDs and coordinates distinguish them; ordinary movement crosses public regions without overview/category map transitions. | Unit duplicate-name, flattened-layer/topology/migration validation and Playwright Song-to-Palos continuous-world tour. |
...
| Safe map schema upgrades | SQLite startup migration | Upgrades populated legacy databases despite SQLite `ALTER TABLE` foreign-key limits, rebuilds schema v5 without the former global location-name uniqueness constraint, and adds schema-v6 per-player exploration history with movement-log/current-location backfill while preserving IDs, foreign-key references and user-created records. | Unit populated-v1/v3 metadata upgrade, duplicate-name, visited-map and foreign-key regression tests. |
| Flat seeded-map storage | `map_layers`, layer-scoped chunks and editor map selector | Keeps only one public `world-root` and one complete `home-ground` single-floor house in the bundled world. Ordinary grid occupancy remains unique per map, while user-created interiors and special maps remain supported. | Unit revision-4 flattening, viewport and transition tests; Playwright two-map inventory. |
...
| Source-tracked complete demo world | SQLite startup / `npm run map:seed` | Idempotently installs 25 sourced house/entrance/street locations, 618 sourced Northern Song nodes and 279 sourced Palos nodes with stable IDs, source URL/version/retrieval date, one continuous public layer, one 21-place single-floor house and full reachability from玄关. Revision 4 deactivates obsolete seeded floors/rooms while preserving revision-0 user content and failing safely on genuine custom grid collisions. | Unit idempotent import, revision-0/2/3 flattening upgrade and `validateWorldMap` topology/source/count tests. |
| Map integrity validation | `npm run map:validate` | Checks layer cycles, the two canonical seed maps, public-location flattening, source records, eight-direction adjacency, reciprocal direction slots, the sole seeded interior transition, training-room effect, 500+/per-world minimums and full reachability. | Unit source-tracked flat-world validation test; command exits non-zero on errors. |
