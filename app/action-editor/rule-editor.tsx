...
export default function ActionRuleEditor({ initialRules }: { initialRules: ActionRuleSnapshot }) {
  ...
  useEffect(() => {
    ...
    socket.addEventListener("open", () => { setConnected(true); setNotice("规则服务已连接。"); });
    socket.addEventListener("close", () => { setConnected(false); setNotice("规则服务连接已断开，请刷新页面重连。"); });
    socket.addEventListener("message", (event) => {
      ...
      if (message.type === "rules.actions.snapshot") {
        ...
        if (nextAction) setForm(toForm(nextAction));
      }
      ...
      if (message.type === "rules.invalidated") socket.send(JSON.stringify({ type: "rules.actions.list", requestId: createClientId() }));
  ...
  return (
    <main className="rule-editor-shell">
      <header className="rule-editor-header">
        <div><p>共享规则 · 所有测试玩家可编辑</p><h1>行动规则设计工具</h1></div>
      ...
      <div className="rule-editor-grid">
        <aside className="rule-list" aria-label="行动规则列表">
          <header><strong>行动模板</strong><button onClick={() => { selectedIdRef.current = ""; setSelectedId(""); setForm({ ...EMPTY_FORM, id: `custom-action-${createClientId()}` }); }}>新增</button></header>
          {rules.actions.map((action) => (
            <button key={action.id} className={selectedId === action.id ? "selected" : ""} onClick={() => { selectedIdRef.current = action.id; setSelectedId(action.id); setForm(toForm(action)); }}>
        ...
        <section className="rule-form" aria-label="行动规则表单">
          ...
          <label>行动名称<input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label>
          <label>行动说明<textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></label>
          <div className="rule-fields-row">
            <label>分类<select value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value as ActionRule["category"] })}>
          ...
          <div className="rule-fields-row">
            ...
            <label className="rule-checkbox"><input type="checkbox" checked={form.adult} onChange={(event) => setForm({ ...form, adult: event.target.checked })} />成人规则</label>
          </div>
          ...
          <div className="rule-json-grid">
            <label>需求 JSON<textarea value={form.requirements} onChange={(event) => setForm({ ...form, requirements: event.target.value })} /></label>
            <label>检定 JSON<textarea value={form.check} onChange={(event) => setForm({ ...form, check: event.target.value })} /></label>
            <label>成本 JSON<textarea value={form.costs} onChange={(event) => setForm({ ...form, costs: event.target.value })} /></label>
            <label>成功结果 JSON<textarea value={form.success} onChange={(event) => setForm({ ...form, success: event.target.value })} /></label>
            <label>失败结果 JSON<textarea value={form.failure} onChange={(event) => setForm({ ...form, failure: event.target.value })} /></label>
          </div>
          <label>结果文本<textarea value={form.resultTemplate} onChange={(event) => setForm({ ...form, resultTemplate: event.target.value })} /></label>
          <div className="rule-form-actions"><button disabled={!connected} onClick={save}>保存规则</button>{form.version !== null && <button disabled={!connected} onClick={() => send({ type: "rules.action.delete", actionId: form.id })}>停用规则</button>}</div>
        </section>
